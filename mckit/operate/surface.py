...
class _MoleculeDetectorLegacy:
    ...
    Stage 1: Build adjacency list of non-metal atoms bonded within
             covalent-radius + tolerance, then BFS to find clusters.
    Stage 2: Filter clusters by spatial extent — only compact groups
             (molecules, not extended networks) are returned.

    Parameters
    ----------
    tol : float
        Bond-distance tolerance beyond covalent radii sum (Angstroms).
    min_size : int
        Minimum number of atoms to count as a molecule.
    """
    ...
    def __init__(self, tol: float = 0.45, min_size: int = 2):
        self.tol = tol
        self.min_size = min_size

    def detect(self, bulk: PmgStructure) -> List[List[int]]:
        ...
        # Build adjacency (non-metals only)
        ...
        for i in range(n):
            ...
            if _is_metal(el_i):
                continue
            ri = _cov_r(el_i)
            for nn in bulk.get_neighbors(bulk[i], cov_cutoff):
                j = nn.index
                if j <= i:
                    continue
                ...
                if _is_metal(el_j):
                    continue
                rj = _cov_r(el_j)
                if nn.nn_distance <= ri + rj + self.tol:
                    adj[i].add(j)
                    adj[j].add(i)
        ...
        # Filter by spatial extent (compact molecules only)
        molecules = []
